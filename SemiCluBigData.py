...
time: 2023-10-10
author: Yongjia Yuan
note: this is semi-supervised cluster algorithm on real-world data set (kdd,letter,....)
"""
...
# perecntage of determine points
D_RATIONS = [
    0.1,
    0.2,
    0.3
...
class SemisupervisedClusteringBigDataAlgo():
    ...
    def __init__(self, data_path='./dataset/', num_clusters=10, max_record=int(1e5)):
    ...
    # def _thresh_choose(self):
    #     if self.nrecord1 <= 5000:
    #         self.thresh = 8.0e-1
    #     elif (self.nrecord1 > 5000) and (self.nrecord1 <= 10000):
    #         self.thresh = 7.0e-1
    #     elif (self.nrecord1 > 10000) and (self.nrecord1 <= 20000):
    #         self.thresh = 6.0e-1
    #     elif (self.nrecord1 > 20000) and (self.nrecord1 <= 35000):
    #         self.thresh = 5.0e-1
    #     elif (self.nrecord1 > 35000) and (self.nrecord1 <= 50000):
    #         self.thresh = 3.5e-1
    ...
    def WholeData(self, Centers,nc):
        wholedata = pd.read_csv("./dataset/dataset_org/%s/%s.txt" %(self.datasetName, self.datasetName), sep=',', index_col=None, header=None).iloc[:, :]
        ...
        #cluster_plan_index_whole, cluster_plan_nk_whole, min_dis_sample_whole, _ = self._assign_data(self.whole_data, Centers,
        #                                                                   self.num_clusters)
        ...
        for i in range(1, self.num_blocks + 1):
            cl_block = pd.read_csv('./dataset/cons_%g/%s/CL/%d.txt' % (self.Link_ration, self.datasetName, i), sep=',', index_col=None, header=None)
            ml_block = pd.read_csv('./dataset/cons_%g/%s/ML/%d.txt' % (self.Link_ration, self.datasetName, i), sep=',', index_col=None, header=None)
        ...
        #np.savetxt('./dataset/cons_%g/%s/CL/whole_cl.txt' %(self.Link_ration, self.datasetName), CL_whole, fmt='%d', delimiter=',')  # save the constraints
        #np.savetxt('./dataset/cons_%g/%s/ML/whole_ml.txt' %(self.Link_ration, self.datasetName) , ML_whole, fmt='%d', delimiter=',')
        ...
        with open('./results/test.txt', 'a') as file:
            file.write(f'nc,cluster_input_x:{nc}\t{Centers}\r\n')
                ...
                for jj in range(0, nc):
                    if j != jj:
                        ...
                        if f_sum > distance_sum:
                            f_sum = distance_sum
                            list1[self.can_whole[i][0]] = j  # 把该点所属的cluster加到list1中
                            list1[self.can_whole[i][1]] = jj
                            min_dis_sample[self.can_whole[i][0]] = distance_cl_1  # 该点到该簇中心的距离
    ...
    def finalresult(self, list1, min_dis_sample, x, nc):
        ...
        if num_labels == 1:
            ...
            sil_avg = silhouette_score(self.whole_data, list1, metric='euclidean') # 返回平均值
            sil_samples = silhouette_samples(self.whole_data, list1, metric='euclidean') # 返回每个点的sil值
            s = [num_pos for num_pos in sil_samples if num_pos > 0]
            #sil_pos = len(s)/self.nrecord  # sil值为正的比例
            sil_pos = len(s)   # sil值为正的个数

        # Normalized mutual information
        NML = normalized_mutual_info_score(self.whole_label, list1)
    ...
    def make_constraints(self, label):
        ...
        num_const = math.floor(len(label) * self.Link_ration)
        ...
        ml_label, cl_label = label_const_list[ml_index[0]], label_const_list[cl_index[0]]
    ...
    def main(self):
        for dataset_name in datasets:
            ...
            for kblocks in range(1, self.num_blocks + 1):
                ...
                self.ML_index, self.CL_index = self.make_constraints(self.Org_label)  # obtain the constraints of the arrived new data
                ...
                #     with open('./results/result_%s%.2f.txt' % (self.datasetName, self.ratio), 'a') as file:
                #         file.write(f'Current time: {time.asctime(time.localtime(time.time()))}''\r\n')
                #     print("results:", f, pur, dbi, sil_avg, sil_pos, NML)
                #
                #
                #     with open('./results/result_%s%.2f.txt' % (self.datasetName, self.ratio), 'a') as file:
                #         file.write(f' The results of the completed dataset''\r\n'
                #                    f' f: {f} \t viol: {viol} \t vcan: {vcan} \t vmust: {vmust} \t pur: {pur} \t '
                #                    f'DBI: {dbi} \t SI_average: {sil_avg} \t SI_pos: {sil_pos} \t NML: {NML}''\r\n')
